...
p <- ggplot(all_waffle, aes(monthweek, weekdayf, fill = freq)) + 
    geom_tile(colour = "white") + facet_grid(~all_waffle$monthf) + scale_fill_gradient(low="orange", high="blue") +  xlab("Week of Month") + ylab("") + ggtitle("Time-Series Calendar Heatmap: Overall Recruitment") + labs(fill = "# Recruited") + scale_shape_binned("square")

p
print(p)
#run individually to ensure the plot looks fine and everything worked so far

ggsave(p, filename = "c:/Recruitment_Waffle/waffle.png",height = 2, width = 10)
