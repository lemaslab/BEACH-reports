...
#renaming normal, and combining overweight and obese to "OWOB"
BMI_bar <- BMI_bar %>%
    mutate(bmi_group = recode(bmi_group, "Normal" = 'NW', "Overweight" = 'OWOB', "Obese" =  'OWOB' ))

#creating counted data for each BMI and month
BMI_bar_counted <- count(BMI_bar)

#cleaning data frame, adding March OWOB freq (as it is 0)
BMI_bar_counted <- na.omit(BMI_bar_counted)
BMI_bar_counted[24,1] <- "OWOB"
BMI_bar_counted[24,2] <- "Mar"
BMI_bar_counted[24,3] <- as.integer(0)

b <- ggplot(BMI_bar_counted, aes(x = factor(month), y = freq, fill = freq, col = bmi_group)) + geom_bar(stat = "identity", position = "dodge") + ylim(0,10) + geom_text(aes(label = freq), vjust = -1)
b

#adjust colors to differentiate
#adjust and y-scale
