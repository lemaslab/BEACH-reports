...
ggsave(a, filename = "c:/waffles/overallRecruitment_waffle.png",height = 2, width = 10)
...
NW_waffle<-ddply(NW_waffle,.(yearmonth),transform,monthweek=1+week-min(week)) # normalizing the week to start at 1 for every month


n <- ggplot(NW_waffle, aes(monthweek, weekdayf, fill = freq)) + 
    geom_tile(colour = "white") + facet_grid(~NW_waffle$monthf) + scale_fill_gradient(low="red", high="green") +  xlab("Week of Month") + ylab("") + ggtitle("Time-Series Calendar Heatmap: Normal Weight Recruitment") + labs(fill = "# Recruited")

n
print(n)
#run individually to ensure the plot looks fine and everything worked so far

ggsave(n, filename = "c:/waffles/BMI_NW_waffle.png",height = 2, width = 10)
#save to computer to adjust size appropriately. adjust filename to where you want it saved
...
OWOB_waffle<-ddply(OWOB_waffle,.(yearmonth),transform,monthweek=1+week-min(week)) # normalizing the week to start at 1 for every month
OWOB_waffle <- na.omit(OWOB_waffle)



o <- ggplot(OWOB_waffle, aes(monthweek, weekdayf, fill = freq)) + 
    geom_tile(colour = "white") + facet_grid(~OWOB_waffle$monthf) + scale_fill_gradient(low="red", high="green") +  xlab("Week of Month") + ylab("") + ggtitle("Time-Series Calendar Heatmap: Overweight and Obese Recruitment") + labs(fill = "# Recruited")

o
print(o)
#run individually to ensure the plot looks fine and everything worked so far

ggsave(o, filename = "c:/waffles/BMI_OWOB_waffle.png",height = 2, width = 10)
#save to computer to adjust size appropriately. adjust filename to where you want it saved

```                               


#Recruitment-Based Waffles
