...
b <- ggplot(BMI_bar_counted, aes(x = factor(month), y = freq, fill = freq, col = bmi_group)) + geom_bar(stat = "identity", position = "dodge") + ylim(0,25) + geom_text(aes(label = freq), vjust = -1)
b

ggsave(b, filename = "c:/waffles/BMI_barplot.png",height = 3, width = 10)
#save to computer to adjust size appropriately. adjust filename to where you want it saved

#adjust colors/patterns to differentiate
