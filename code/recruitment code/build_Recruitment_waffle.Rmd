...
amazonstockg <- count(all$time) %>% ungroup()
names(amazonstockg) [1] <- "time"
...
amazonstockg$weekday = as.POSIXlt(amazonstockg$time)$wday #finding the day no. of the week
amazonstockg$weekdayf<-factor(amazonstockg$weekday,levels=rev(0:6),labels=rev(c("Sun","Mon","Tue","Wed","Thu","Fri","Sat")),ordered=TRUE) # converting the day no. to factor

amazonstockg$monthf<- 1

amazonstockg$monthf<-factor(month(amazonstockg$time),levels=as.character(1:12),labels=c("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"),ordered=TRUE) # finding the month

amazonstockg$yearmonth<- factor(as.yearmon(amazonstockg$time)) # finding the year and the month from the date. Eg: Nov 2018
#might not need this

amazonstockg$week <- as.numeric(format(amazonstockg$time,"%W")) # finding the week of the year for each date

amazonstockg<-ddply(amazonstockg,.(yearmonth),transform,monthweek=1+week-min(week)) # normalizing the week to start at 1 for every month


p <- ggplot(amazonstockg, aes(monthweek, weekdayf, fill = amazonstockg$freq)) + 
    geom_tile(colour = "white") + facet_grid(~amazonstockg$monthf) + scale_fill_gradient(low="red", high="green") +  xlab("Week of Month") + ylab("") + ggtitle("Time-Series Calendar Heatmap: Overall Recruitment") + labs(fill = "# Recruited")
