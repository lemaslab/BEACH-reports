...
# REPORT IDS
#-----------
#*** denote the only things you should need to change
record_id <- "BLS038A"  #***Replace with the target record ID
field_name <- "dietrecord_txt_output" #this calls the variable/field that holds the text_file diet record/annotation - shouldn't need to be changed
field_name_output <- "dietrecord_csv_redcap" #this is the variable/field where it will be uploaded - shouldn't need to be changed
event_name <- "six_month_arm_1" #***replace if different event
...
#notes the name of the file, so it can be different from the initial
...
red_file_name <- downloaded_file[["file_name"]]
#format txt file into dataframe
txt_file <- read.table(red_file_name, skip = 2, header = FALSE, sep = "\t", quote = "")

#these sort the naming for the output/upload file for later, but also add event_name_short to data file
...
#assign(file_name, read.csv(red_file_name))
...
#uploads long dataset to diet_record_processed
