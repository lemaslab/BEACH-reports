  ...
  select(test_id,
   ...
   mutate(gest_wks_groups= case_when((gestation_wks <= 10) ~ "0-10",
                                 ...
                                 (gestation_wks >30 & gestation_wks <= 40) ~ "31-40",
                                  (gestation_wks > 40) ~ "> 40"))%>%
  select(test_id, Prescreen_Survey,Phone_Screen, IDR, Gest_wks,gest_wks_groups, GNV, Prego, Mod)
...
```{r, message=FALSE,warning=FALSE, echo = FALSE}
#pregnant for mode of delivery for phone screen
