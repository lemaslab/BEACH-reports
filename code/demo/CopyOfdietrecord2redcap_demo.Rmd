...
record_id <- "BLS006A"  #***Replace with the target record ID
...
event_name <- "third_trimester_arm_1" #replace if different event 

#fixes naming issue with redcap_download_file
event_name_short <- strsplit(event_name, "_")[[1]][1:2] 
event_name_short <- paste(event_name_short, collapse = "_")
file_name <- glue("{record_id}_{event_name_short}.txt")

#download file directly from Redcap to R with API token
redcap_download_file_oneshot(file_name = file_name, redcap_uri = uri, token = api_token, record = record_id, field = field_name, event = event_name)

#format txt file into dataframe
txt_file <- read.delim(file_name, skip = 2, head=FALSE) #header is skipped so it doesn't weirdly format the first row of data as a "header" and so this script can be continuous with the diet2redcap function
...
# Get a list of all files in the current working directory (cwd) that match the pattern
file_list <- list.files(pattern = "BLS\\d+A_third_trimester_redcap_ready.csv")
...
for(file_name in file_list) {
  ...
  new_file_name <- gsub("third_trimester", "third_trimester_arm_1", file_name)
  file.rename(file_name, new_file_name)
  
  # Task 2: Change the "redcap_event_name" column
  df$redcap_event_name <- "third_trimester_arm_1"
  
  # Task 3: Rename the "food_measure_quantity" column
  colnames(df)[colnames(df) == "food_measure_quantity"] <- "food_record_quantity"
