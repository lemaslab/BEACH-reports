---
title: "getData_current_particpant"
author: "Yasmine Gillespie"
date: '2022-10-23'
output: html_document
---


```{r, include=FALSE}
##--------------
# **************************************************************************** #
# ***************                Project Overview              *************** #
# **************************************************************************** #
# Author:      Hailey Ballard
# Date:        October 21, 2021
# IRB:         The Breastfeeding and EArly Child Health (BEACH) study (IRB201601034)
#
# version: R version 4.0.2 (2020-06-22)
# version: Rstudio version Version 1.3.959
# **************************************************************************** #
# ***************                Description                   *************** #
# **************************************************************************** #
# PLAN: Access redcap via API and pull/format/clean data for downstream analysis.
# Note: vpn must be active
```


```{r, message=FALSE}
# **************************************************************************** #
# ***************                Library                       *************** #
# **************************************************************************** #
library(keyringr)
library(redcapAPI)
library(REDCapR)
library(readxl)
library(tidyverse)
```

```{r, message=FALSE}
# Windows
source("~/BEACH-facebook/code/utils/utils.R")
api_token=get_API_token("BEACH-Facebook")

#source("~/BEACH-reports/code/utils/utils.R")
#api_token=get_API_token("BEACH-reports")
```

```{r, message=FALSE}
# API and URL
uri='https://redcap.ctsi.ufl.edu/redcap/api/'
rcon <- redcapConnection(url=uri, token=api_token)
report_id <- 35366
# Access data
CP_raw=redcap_report(
  uri,
  api_token,
  report_id,
  raw_or_label_headers="raw",
  guess_type=FALSE)$data
#changing one participant's info that can't be inputted in RedCap
CP_raw[2733,16] <- "Flyer"


#save(list=c("CP_raw"),file="~/BEACH-reports/data/raw/Current_Particpant_raw.RData")
save(list=c("CP_raw"),file="~/BEACH-facebook/data/raw/Current_Particpant_raw.RData")
```

