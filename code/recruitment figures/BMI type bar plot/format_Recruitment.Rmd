...
load("~/BEACH-Reports/data/raw/Recruitment_Particpant_raw.RData")
load("~/BEACH-reports/data/raw/Current_Particpant_raw.RData")

```

```{r, message=FALSE,warning=FALSE, echo = FALSE}
recruitment_report <- Recruitment_raw[ -c(2:4) ]
```
...
recruitment_report$timecoalesce = coalesce(recruitment_report$prescreen_date, recruitment_report$beach_encounter_date, recruitment_report$beachphone_date)
...
CP_bmi = CP_raw %>%
   mutate(BMI = case_when((BMI >= 29) ~ "Obese",
                                  (BMI >= 26 & BMI<29) ~ "Overweight",
                                  (BMI <26) ~ "Normal"))
...
BMI_barplot <- cbind(recruitment_report, CP_bmi$BMI)
...
save(list=c("BMI_barplot"),file="~/BEACH-Reports/output/reports/recruitment/BMI_barplot.RData")
