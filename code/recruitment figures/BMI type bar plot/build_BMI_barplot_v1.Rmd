...
CP_raw = CP_raw %>%
   mutate(BMI = case_when((BMI >=26) ~ "OWOB", (BMI >= 18.5 & BMI<26) ~ "NW", (BMI < 18.5) ~ "UW"))
...
BMI_UW = filter(BMI_waffle, bmi_group =="UW")
...
sum(UW_months$month == 'Oct' | UW_months$month == 'Nov' | NW_months$month == 'Dec')
...
ggsave(b, filename = "BMI_barplot.png",height = 6, width = 10)
